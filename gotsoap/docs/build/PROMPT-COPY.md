...
1. **`gotsoap/docs/copy/voice-bible.md`** — the register. Contains the **Stepford test** (below),
   the dual-address engine, and sound-like / doesn't-sound-like exemplars. Read it first.
2. **`gotsoap/docs/copy/copy-contract-for-build.md`** — the non-negotiable invariants + the
...
`gotsoap/docs/copy/style-lock.md` capturing any voice decision you had to make so the next copy
session inherits it.
...
3. **Write the draft into `gotsoap/docs/copy/copy-session-draft.md`** with, per line: the option(s),
...
LAW above, then audit `gotsoap/docs/copy/copy-session-draft.md`:
...
- **Round cap: 3.** If a single line isn't settled in three exchanges, stop. Put both
  versions + both arguments in `gotsoap/docs/copy/copy-session-draft.md` under "For Stacey" and move
