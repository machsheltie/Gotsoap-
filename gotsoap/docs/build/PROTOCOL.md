...
| **Codex Sol 5.6** | Adversarial Critic | Reads the worktree, runs greps, writes review reports under `gotsoap/docs/build/reports/`. | **Never edits `site/src`.** Never proposes patches. Names violations; does not fix them. |
...
```
  ...
  │
  ├─ 3. Fable writes  gotsoap/docs/build/reports/phase-N-build.md
  │       └─ pastes the `gate-set:` fingerprint verbatim
  │
  ├─ 4. Sol AUDITS    → gotsoap/docs/build/reports/phase-N-review.md
  │       └─ per-finding PASS / FAIL / CONCERN, each with file:line + spec clause
  │
  ├─ 5. Any FAIL or CONCERN?
  │       └─ Fable remediates → gotsoap/docs/build/reports/phase-N-remediation.md
...
the copywriter session, now defined in **`gotsoap/docs/build/PROMPT-COPY.md`** — a Claude⇄Sol
...
Reports live in `gotsoap/docs/build/reports/`. They are the bus between two models that
