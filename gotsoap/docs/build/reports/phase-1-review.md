...
| Surface | Ruling |
|---|---|
| Legacy home and hero wash | Phase 3 owns `/movement-preview` → `/` and the home signatures (`gotsoap/docs/build/PROTOCOL.md:60`). The live `/` is already Hero-only at `src/pages/index.astro:27`; the five-poster `Spot.astro` is dead code, not the live pilgrimage Fable described. Its cleanup remains Concern 1. G7/G7b correctly defer the canonical hero washes to Phase 3. |
| Shared detail split and crossing lightbox control | Phase 2 owns all of §5/§6, the five environment components, and replacement of `[slug].astro` with a pure orchestrator (`gotsoap/docs/build/PROTOCOL.md:59`). The shared split and the absolutely positioned control at `src/pages/psas/[slug].astro:90` cannot survive that replacement. |
| Staggered `/psas` gallery | Phase 5 owns the placement-hub replacement (`gotsoap/docs/build/PROTOCOL.md:62`). G8 correctly remains deferred. |
