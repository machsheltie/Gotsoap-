...
**Scope:** every user-facing string on the site, judged against `gotsoap/docs/copy/CAMPAIGN-INTENT.md`.
...
1. **`gotsoap/docs/copy/CAMPAIGN-INTENT.md`** — the thesis, the two audiences, the aspiration mechanic, the
   bar, and the 10 judgment criteria. **This is the standard.**
2. **`gotsoap/sources/promo.txt`** + **`gotsoap/sources/gotsoapdescription.txt`** — the origin texts, in the campaign's own words.
...
4. **`gotsoap/docs/copy/voice-bible.md`** (register + Stepford test) and **`style-lock.md`** (locked
...
Answer, in a short findings note (`gotsoap/docs/copy/research-satirical-campaigns.md`), with citations:
...
**→ `gotsoap/docs/copy/reader-sim-evidence.md` — READ IT BEFORE YOU ARGUE ANYTHING.**
...
Write `gotsoap/docs/copy/copy-review-findings.md`: verdict, findings ranked by severity, and — **required** —
...
Sol's Phase 3 audit landed (`gotsoap/docs/copy/copy-review-findings.md`): 17 findings, 3 P0 blockers. Owner
ruling on how Phase 4 consumes them:

- **F1 (Vector-2 citation path) is OUT of the dialectic.** It's a net-new feature with a
  harassment-safety surface → specced separately (`cwaaa/docs/copy/vector2-citation-path-spec.md`) for its
