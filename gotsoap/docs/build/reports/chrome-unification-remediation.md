...
| The deferral now has an owner | **PROMPT-COPY.md work item 10** (new row): voice the CWAAA close in the org's register or ratify the plain default; explicitly instructs the lane that the rejected "Return to the document" has no standing as a draft. The copy floor is already acceptance-blocking at the Phase 6 launch audit (PROTOCOL, "the copy lane"), so this is deferred-to-somewhere with a gate in front of it. Item 9 also updated: `nav.links`/`nav.quiet` orphaning noted for the lane. | `gotsoap/docs/build/PROMPT-COPY.md` |
