...
Companion to `gotsoap/docs/research/got-milk-campaign-analysis.md`. That doc covers strategy (why Got Milk? worked, the Liquid Death/CAH playbook, site architecture). This doc is the **visual design plan**: the token system, layout concept, and signature element for the site itself.
...
- **`gotsoap/sources/promo.txt` line 74 — a road not taken:** Stacey considered giving the headline "a frosted over look like you might see on a fogged up shower door" and dropped it from the posters for legibility reasons. That unused idea is exactly the kind of subject-specific material the signature element should come from — steam and fogged glass are the literal physical mechanism of "getting clean," and it's already latent in this project's own history rather than borrowed from nowhere.
- **The type spec in `gotsoap/sources/promo.txt`** is prescriptive (Phenix American, Futura PT, Franklin Gothic ATF Heavy, Montserrat) — the job here is to honor it on the web, not replace it with a generic display/body pairing.
...
### Type — matches `gotsoap/sources/promo.txt`'s spec, decided (not silently substituted)
