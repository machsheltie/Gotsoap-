...
- **OWNER DECISION 2026-07-15: the flagship is Unholy** — maximum stopping power and the campaign's sharpest provocation. `HOME_FLAGSHIP` in `config/site.ts` must read `'unholy'`; it currently reads `'thirst-announcement'` and is a live mismatch to fix. The choice does not alter the route architecture.
...
> **OWNER AMENDMENT 2026-07-15 (supersedes the original §3.7 above-the-fold placement).**
> The masthead maker credit is retired. Reason: at the top of the page it tips the
> satire before the campaign lands — most sharply for the warm audience arriving from
> Stacey's socials, who already know her as the satirist. The fiction-first premise
> ("this must read as a real campaign") outranks the 45-second-skim lead-gen argument;
> engaged visitors still reach the maker via the footer and the `/about` proof-lift,
> which is the real portfolio payload. §14's above-the-fold gate line is struck to match.

- Keep `/about` subordinate but findable.
- **The masthead carries no real-world maker credit.** It stays pure in-fiction — the
  wordmark and the Vogue-style issue-number gag ("No. 26") read as an authentic fashion
  masthead. No "PRODUCED BY…" line above the fold.
- Add this exact in-fiction credit to the **footer only**: **PRODUCED BY HOPE2 STUDIO ·
  DIRECTED BY STACEY BRECKEL**. Small caps with middots, linked to `/about`. In the
  footer it reads as an authentic campaign credit, not a punchline pre-empt.
> **OWNER AMENDMENT 2026-07-16 (commit-to-the-bit; supersedes the two bullets below).**
> The home commits fully to the fiction, Onion-style — no satire tell anywhere on the home.
> Three prior `/about` tells are struck: the footer `credit`'s "· A satirical spec campaign…",
> and the Reveal beat's `tail` ("Got Soap? is a satirical campaign by Hope2 Studio") and `cta`
> ("See who's behind it →"). Consequences, now binding:
> - **The parody disclaimer moves off the global footer to `/about`**, reworded as neutral
>   non-affiliation ("Parody. Not affiliated with any brand, product, publication, or
>   public-health organization…"). Non-affiliation is legal fine print; it is not the "satire"
>   reveal, and it lives at the reveal page.
> - **The Reveal beat keeps only its thesis line** — "None of these men are real. The problem is
>   very, very real." — as a quiet `<h2>` with no route and no meta. The full reveal lives at `/about`.
...
- The folio credit literal stays hardcoded in `Footer.astro` (it is approved-verbatim and gate G11
  ...
  through `copy.ts`."
- `/about` remains the full fourth-wall break; the footer credit is the only maker attribution on
  campaign surfaces.
...
4. **Flagship section background** (added 2026-07-16) — a purpose-built atmospheric image for the home's Unholy Campaign section, in the smoke register, dark and uniform enough that the `.fx-gold` headline and body copy clear WCAG AA. **Not** the canonical poster reused (a blurred flagship reads as muddy and starves the gold text of contrast; G7c now blocks it). Until it lands, the section uses a CSS smoke-material ground; production ship of that section waits on the real asset.

## Decisions reserved for the owner

1. ~~Choose the home flagship: Poster 5 or Unholy.~~ **RESOLVED 2026-07-15 — Unholy.**
   See §3.3. `config/site.ts` → `HOME_FLAGSHIP = 'unholy'`.
